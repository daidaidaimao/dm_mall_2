...
<script>
  ...
  import {uploadFileRequest} from '@/utils/api.js' //接口url配置文件
      ...
      finish(type) {
        ...
        if (type === 'blob') {
          this.$refs.cropper.getCropBlob((data) => {
            ...
            this.model = true;
            this.modelSrc = img;
           formData.append("pic",  this.fileName);
           //  formData.append("pic",type.target.files[0]);
            // this.$http.post(Api.uploadSysHeadImg.url, formData, {contentType: false, processData: false, headers:{'Content-Type': 'application/x-www-form-urlencoded'}})
              uploadFileRequest("/product/picupload",formData).then((response)=>{
